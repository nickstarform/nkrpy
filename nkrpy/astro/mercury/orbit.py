...
# relative modules
from ...io.load import load_cfg, verify_dir
from ...io.file_loader import parse_aei

# global attributes
__all__ = ('main',)
__doc__ = """
This packages tries to be fairly robust and efficient,
utilizing the speedups offered via numpy where applicable
and multicore techniques. To get started, simply need a config
file and call orbit.main(config). Inside the config should be
mostly 3 things: files<input file list> out_dir<outputdirectory>
and out_name<unique output name>. A lot of files will be generated
(sometimes tens of thousands). The end goal is matplotlib libraries
are ineffient for animation creation, so static thumbnails are created
and then a imagmagick shell script is created to utilize a more
efficient program.
"""
...
def main(cfgname):
    ...
    if os.path.isfile(f'{dest}/{cfg.out_name}.dat') and\
       os.path.isfile(f'{dest}/{cfg.out_name}.head'):
...
def print_cmd(dest, cfg, length, chunk=100):
    ...
    with open(f'{cfg.out_name}_gifgen.sh', 'w') as f:
        for x in range(length):
            ...
            if ((x % chunk) == (chunk - 1)) or\
               (x == (length - 1)) and (_t != ''):
                f.write(f'convert -loop 0 -delay 20 ' +
                        f'{_t} {dest}/chunk_{count}.gif &\n')
                if (count % cpu) == (cpu - 1):
                    f.write(f'echo -ne "Progress:' +
                            f'[{count + 1}/{length / chunk}]"\\\\r\n')
                    f.write('wait\n')
                _t = ''
                count += 1
        f.write(f'wait\n')
        f.write(f'echo "Only Chunks of gifs are made, run the ' +
                f'last command yourself:' +
                f'`echo {cfg.out_name}_gifgen.sh | tail -n1`"\n')
        f.write('# The next command is commented out because can eat RAM\n')
        f.write(f'# convert -loop 0 -delay 20 ' +
                f'{dest}/chunk_*.gif {dest}/{cfg.out_name}.gif\n')
    print(f'Run the command: {cfg.out_name}_gifgen.sh')
    print('I commented out the last line as' +
          'it can take a shocking amount of RAM')
    pass
