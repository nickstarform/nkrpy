...
def dustmass(dist=100, dist_unit="pc", val=0.1,
    ...
    """
    dist = unit(baseunit=dist_unit,
                vals=dist)("cm")  # to match the opacity units
    wav = unit(baseunit=val_unit,
               vals=val)("microns")  # to search opcaity models
    intensity = planck_nu(temp,
                          unit(baseunit=val_unit,
                               vals=val)("hz"), "hz") * 1.E26  # noqa in jansky
    # embed()
    if not opacity:
        opacity = kappa(wav,
