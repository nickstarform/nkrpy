...
__all__ = ('test', 'main')
__doc__ = """
Handles bulk reduction for ARCSAT.
Must have a config file defined and tries to do basic reduction quickly.
"""
...
def main(cfgname, clear=False):
    ...
    os.chdir(cfg._cwd)
    cal(cfg, science_i, bias_image=masterbias,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', '--gen', dest='g',
                        default=False,
                        action="store_true",
                        help='generate config')
    parser.add_argument('-r', '--resume', dest='r',
                        default=False,
                        action="store_true",
                        help='pickup reduction where left off')
...
read in config file
config has flats, darks, bias, science, createplot,
    createfits, destination, color
