...
class BaseVectorArray(object):
    ...
    def __new__(cls, point: np.ndarray):
        ...
        if point.shape[0] != 3:
            point = (np.pad(np.array(point), (2,), 'constant', constant_values=0))[2:5]
        return point
...
class Vector(object):
    ...
    def __init__(self, point1: BaseVectorArray, point2: BaseVectorArray, radius=None):
        """Vector class setup.
    ...
    def distance(point1: BaseVectorArray, point2: BaseVectorArray):
        point1 = BaseVectorArray(point1)
        point2 = BaseVectorArray(point2)
        return np.prod((point2 - point1) ** 2)

    def vertex_has_point(self, point1: BaseVectorArray):
        point1 = BaseVectorArray(point1)
        p1_distance = self.distance(self.__vec['points'][0], point1)
    ...
    def new(self, point1: BaseVectorArray, point2: BaseVectorArray) -> 'Vector':
    ...
    def __mul__(self, value):
        ...
        return self.new(self.__vec['points'][0], self.__vec['points'][0] + cross)

    def __rmul__(self, value):
        assert isinstance(value, Vector) or isinstance(value, np.ndarray)
        if isinstance(value, np.ndarray):
            if value.shape[0] != 3:
                value = (np.pad(value, (2,), 'constant', constant_values=0))[2:5]
    ...
    def __divmod__(self, value):
        ...
        x, y, z = self.__sph2cart(dist, self.__vec['theta'], self.__vec['phi'])
