...
def icrs2degrees(icrs):
    ...
    icrs = [float(i) for i in icrs if i != '']
    ret = sum([v / 60 ** i for i, v in enumerate(icrs)])
    return ret


cpdef degrees2icrs(deg):
    ...
    cdef cnp.ndarray[double, ndim=1, mode='c'] ret = np.ones(3, dtype=np.float64)
    ret[0] = deg - deg % 1
    ret[1] = ((deg % 1) * 60) 
    ret[2] = (ret[1] % 1) * 60
    ret[1] = ret[1] - (ret[1] % 1)
