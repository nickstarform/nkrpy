...
# relative modules
from nkrpy.io import _fits as fits
...
class File(object):
  """ An helper class for file reading
      ...
      with File('file.name') as f:
         ...
         for row in f.backward():
             print row """
  __files = {}
  SUPPORTED_FILES = ('fits', 'txt')

  def __new__(cls, filename):
    if filename in cls.__files:
      return cls.__files[filename]
    instance = object.__new__(cls)
    cls.__files[filename] = instance
    return instance
  ...
  def __init__(self, filename, *args, **kwargs):
    ...
    self.BLOCKSIZE = 4096
    if ext == 'fits':
      # shadow functions with fits
      self.head = None
      self.tail = None
      self.read = fits.read
      self.write = fits.write
      self.functions = fits

  def open(self, action: str = 'r'):
    self.file = open(fname, action)
